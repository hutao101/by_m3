...
/*******************************************************************************
 * @brief   sample_i2c_t17_1
* Function  Interrupt_I2C2:RX_OVER
 * @author  licuihuang
 * @date    2022-4
 * @param   void
 * @return  void
...
void sample_i2c_t17_1(void)
{
	...
	write_ahb32(i2c2_info.base_address +  I2C_INTR_MASK,status & I2C_RX_OVER);             //[1]：I2C_RX_OVER,1，
	...
	write_ahb32(i2c2_info.base_address +  I2C_DATA_CMD,I2C_CMD );  
	delay_ms(1); //不延时会产生中断[6]:TX_ABRT,I2C_ABRT_SOURCE:0X802000,即显示正在刷新TX_FIFO
	snps_i2c_write_1(&i2c1_info);
	
		for(i=0x1;i<=0x12;i++)
	{
		delay_ms(1);
		write_ahb32(i2c2_info.base_address +  I2C_DATA_CMD,I2C_CMD);  //[1]:RX_OVER 1
	}	
