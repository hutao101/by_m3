...
{  
	...
	TIM_OCInitStructure.TIM_OCIdleState = TIM_OCIdleState_Reset; //CH1输出空闲状态电平与CH1N相反，要不两通道都输出0
	TIM_OCInitStructure.TIM_OutputNState = TIM_OutputNState_Enable;
...
//	GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化GPIO
    ...
    TIM_SelectInputTrigger(TIM2, TIM_TS_TI2FP2);	 // 选择输入捕获的触发信号 TIM_TS_TI1FP1		
    TIM_SelectSlaveMode(TIM2, TIM_SlaveMode_Reset);  // 选择从模式: 复位模式,PWM输入模式时,从模式必须工作在复位模式，当捕获开始时,计数器CNT会被复位
...
//    TIM_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Rising;  //选择极性时就把捕获使能开了CCER[0]
    ...
    TIM_ICInitStructure.TIM_ICPrescaler = TIM_ICPSC_DIV1;
    TIM_ICInitStructure.TIM_ICFilter = 0x0;  //0xf
  	TIM_ICInit(TIM2,&TIM_ICInitStructure);	
}
...
//				Frequency = (50000000/(9999+1))/(float)(IC1Value+1);   /* 频率计算 */
...
/********输入捕获-单边沿*********/
		  if(i == 0)
			...
			{				
				Frequency = (float)(50000000/((9999+1)*((TIM_GetCapture1(TIM1)+1)-(IC1Value+1))));
...
//		if(TIM_GetFlagStatus(TIM2,TIM_FLAG_CC1OF))
//		{
//			printf("TIM2 CC1OF IRQ!!\r\n");
//			TIM_ClearITPendingBit(TIM2, ~TIM_IT_CC1OF); //对CC1OF写0清标志位
//		}		

//		if(TIM_GetFlagStatus(TIM2,TIM_FLAG_CC2OF))
//		{
//			printf("TIM2 CC2OF IRQ!!\r\n");
//			TIM_ClearITPendingBit(TIM2, ~TIM_IT_CC2OF  );  
//		}
//		
//		if(TIM_GetFlagStatus(TIM2,TIM_FLAG_CC3OF)) 
//		{
//			printf("TIM2 CC3OF IRQ!!\r\n");
//			TIM_ClearITPendingBit(TIM2, ~TIM_IT_CC3OF  );  
//		}
//		
//		if(TIM_GetFlagStatus(TIM2,TIM_FLAG_CC4OF))
//		{
//			printf("TIM2 CC4OF IRQ!!\r\n");
//			TIM_ClearITPendingBit(TIM2, ~TIM_IT_CC4OF  );  
//		}
//		if (TIM_GetITStatus(TIM2, TIM_IT_CC1) != RESET) 
//		{
//			printf("TIM2 CC1 IRQ!!\r\n");
...
//			printf("TIM2 CC2 IRQ!!\r\n");
//		  TIM_ClearITPendingBit(TIM2, TIM_IT_CC2  ); 
//			printf("%d ",read_ahb32(0x40018038));
//			printf("%d ",read_ahb32(0x40018038));
//			printf("%d ",read_ahb32(0x40018038));
//			printf("%d ",read_ahb32(0x40018038));
//			printf("%d ",read_ahb32(0x40018038));
//			printf("%d\r\n",read_ahb32(0x40018038));
...
////			printf("占空比：%0.2f%%   频率：%0.2fHz\r\n",DutyCycle,Frequency);
////通道2：
//			 DutyCycle = (float)((IC1Value+1) * 100) / (IC2Value+1);  /* 占空比计算 */
//			 Frequency = (36000000/(9999+1))/(float)(IC2Value+1);   /* 频率计算 */
//			 printf("占空比：%0.2f%%   频率：%0.2fHz\r\n",DutyCycle,Frequency);				
//		}
//		else
//		{
...
//				{
//					Frequency = (float)(36000000/((999+1)*((IC2Value+1)-(IC1Value+1))));
//				  printf("IC2Value:%d  IC1Value:%d  fre:%f\r\n",IC2Value,IC1Value,Frequency);
//				}
//				else if(IC2Value==IC1Value)
//					{
//						printf("IC2Value:%d  IC1Value:%d error\r\n",IC2Value,IC1Value);
//					}	
//					else
//					{
//						Frequency = (float)(36000000/((999+1)*(65536+(IC2Value+1)-(IC1Value+1))));
//					  printf("IC2Value:%d  IC1Value:%d  fre:%f\r\n",IC2Value,IC1Value,Frequency);				  
//					}
//					IC1Value = IC2Value;
//					IC2Value = TIM_GetCapture2(TIM2);					
//					Frequency = (36000000/(999+1))/(float)((IC2Value+1)-(IC1Value+1));
//					printf("IC2Value:%d  IC1Value:%d  fre:%f\r\n",IC2Value,IC1Value,Frequency);
//					IC1Value = IC2Value;
//			}
/********输入捕获-单边沿*********/
...
{
//	if(TIM_GetITStatus(TIM1, TIM_IT_CC1OF) != RESET) //
	{
		TIM_ClearITPendingBit(TIM1, TIM_IT_CC1OF);
	  	printf("  CC1OF IRQ!!\r\n");	
	}
}
...
{
//	if(TIM_GetITStatus(TIM1, TIM_IT_CC2OF) != RESET) 
	{
		TIM_ClearITPendingBit(TIM1, TIM_IT_CC2OF);
	  	printf("  CC2OF IRQ!!\r\n");	
	}
}
...
{
//	if(TIM_GetITStatus(TIM1, TIM_IT_CC3OF) != RESET) 
	{
		TIM_ClearITPendingBit(TIM1, TIM_IT_CC3OF);
	  	printf("  CC3OF IRQ!!\r\n");	
	}
}
...
{
//	if(TIM_GetITStatus(TIM1, TIM_IT_CC4OF) != RESET) 
	{
		TIM_ClearITPendingBit(TIM1, TIM_IT_CC4OF);
	  	printf("  CC4OF IRQ!!\r\n");	
	}
}
