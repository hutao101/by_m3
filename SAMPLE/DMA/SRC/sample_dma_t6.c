...
int dmac_16bit_iic2mem_cfg(int channel)
{
    u32 reg_value; 
...
/****************************************************************************
 * @brief   sample_dma_t6
 * Function test the IIC to SRAM dma transfer of des is 16bit、des is 8bit for per ch
 * @author  he
 * @date    2022-04-02
 * @param   
...
void sample_dma_t6(int channel)
{
	
	dw_dmac_clearIrq(channel, IRQ_TYPE_ALL);
	dmac_16bit_iic2mem_cfg(channel);
