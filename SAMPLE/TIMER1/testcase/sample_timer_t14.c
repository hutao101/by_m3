...
/*******************************************************************************
 * @brief   sample_timer_t14   
 * Function 霍尔传感器(tim4通道1/3输入 通道2触发 通道1/3悬空)
 * @author  licuihuang
 * @date    2022-6-2
 * @param   void
 * @return  void
...
{
//无tim3
//测tim1：2.5hz的逻辑分析仪输出接tim2_ch1(输入捕获模式)，ch2和ch3(pwm2触发输出模式)悬空，tim1_ch1为pwm1输出模式
//根据tim2_ch1波形推tim1_ch1波形
	u32 i=3; 
//	for(i=2;i<4;i++) //CH2/3接连做触发输出跑出来波形不对,分开跑
	{
	//	TIM_GPIO_Init();
