...
void sample_timer_t22_fordma_t23(void)
{
...
int dmac_32bit_timer2mem_cfg_t23(int channel)
{
    u32 reg_value; 
...
void sample_dma_t23(int channel)
{
...
/*begin*****************************顺序一：DMA配置及使能、TIM配置及使能、握手*****************************************/	
	...
	dmac_32bit_timer2mem_cfg_t23(channel);  //DMA 初始化
	dw_dmac_enableChannel(channel);
	
	
	printf("3333\r\n");
	sample_timer_t22_fordma_t23();
...
//	TIM1->DIER |= 1<<11;  // Capture/Compare 1 DMA request enable
//	
//	dw_dmac_clearIrq(channel, IRQ_TYPE_ALL);
//	dw_dmac_unmaskIrq(channel,IRQ_TYPE_ALL);
//	
