...
/*******************************************************************************
 * @brief   sample_timer_t14_1   
 * Function 霍尔传感器(tim4通道1悬空 通道2输入 通道3触发)
 * @author  licuihuang
 * @date    2022-6-2
 * @param   void
 * @return  void
...
{
//无tim3
//测tim1：2.5hz的逻辑分析仪输出接tim2_ch3(pwm2触发输出模式)，ch1(输入捕获模式)和ch2悬空，tim1_ch1为pwm1输出模式
//根据tim2_ch1波形推出tim1_ch1波形
	u32 i=3; 
//	for(i=2;i<4;i++) //用for使CH2/3接连做触发输出跑出来波形不对,分开跑
	{
	//	TIM_GPIO_Init();
