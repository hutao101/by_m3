...
/*******************************************************************************
 * @brief   sample_i2c_t20_1
* Function  Interrupt_I2C2:I2C_MST_ON_HOLD(fail)
 * @author  licuihuang
 * @date    2022-3
 * @param   void
 * @return  void
...
void sample_i2c_t20_1(void)
{
	...
	write_ahb32(i2c2_info.base_address +  I2C_INTR_MASK,status & I2C_MST_ON_HOLD);    //[13]：I2C_MST_ON_HOLD
	
	snps_i2c_write_1(&i2c2_info);		 //I2C（主从）活动	
	
	delay_ms(10);
	write_ahb32(i2c2_info.base_address +  I2C_DATA_CMD,0x56|I2C_RESTART );	
	delay_ms(2);
		
	printf("I2C_RAW_INTR_STAT:0x%x  0x%x\r\n",read_ahb32(i2c2_info.base_address +  I2C_RAW_INTR_STAT),read_ahb32(i2c2_info.base_address +  I2C_RAW_INTR_STAT));	
	printf("I2C_CON:0x%x  I2C_TAR:0x%x\r\n",read_ahb32(i2c2_info.base_address +  I2C_CON),read_ahb32(i2c2_info.base_address +  I2C_TAR));	
	printf("I2C_DATA_CMD:0x%x  0x%x\r\n",read_ahb32(i2c2_info.base_address +  I2C_DATA_CMD),read_ahb32(i2c2_info.base_address +  I2C_DATA_CMD));	
	printf("\r\n");
	snps_i2c_close(&i2c1_info);
	snps_i2c_close(&i2c2_info);
}
