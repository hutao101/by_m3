...
//m 2 p 32bit ccr3 pwm

void sample_timer_t22_toDma_t27(void)
{
	TIM1_NVIC_Init(33);
...
int dmac_32bit_mem2iic_cfg_t27(int channel)
{
    u32 reg_value; 
	...
	reg_value =   (1 << 20)	   										// memory to peripheral ,control by ahb-dmac
				 ...
				 | (0 << 11)| (1 << 14)		         //source burst length is 4	and dest also is 4 
...
//	printf("MCU_CFG: 0x%x\r\n",read_ahb32(MCU_CFG));
	...
	reg_value =    (0 << 7)| (5 << 11)		//Handshaking 5:TIMER1_CC3 MCU_CFG[14:12]=100			
...
void sample_dma_t27(int channel)
{
/*begin******************************顺序一 ：DMA 握手 TIM***************************/
	...
	TIM1->DCR |= TIM_DMABase_CCR3; //DMA base address = TIM_DMABase_CCR1
