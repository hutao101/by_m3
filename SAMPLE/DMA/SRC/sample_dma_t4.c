...
//m2m 16bit
//t4测试代码
//测试8个通道SRAM 2 SRAM16bit传输是否正常
//测试结果：8通道16bit正常搬运


int dmac_16bit_mem2mem_cfg(struct mem2mem_info *mem2mem, int channel)
{
    u32 reg_value;  
...
/****************************************************************************
 * @brief   sample_dma_t4
 * Function test the SRAM to SRAM dma transfer of 16 bit for per ch
 * @author  he
 * @date    2022-04-02
 * @param   
 ...
 * @return void
******************************************************************************/
void sample_dma_t4(int channel)
{
	...
	mem2mem_info_cfg.des_width = 1;
	mem2mem_info_cfg.src_width = 1;
	mem2mem_info_cfg.block_size = 21;   //Block_TS  一个块的数据数量
	mem2mem_info_cfg.trans_length_inbytes = 42;   //传输块的长度（字节为单位）
	mem2mem_info_cfg.scatter .enable  = 0;
	mem2mem_info_cfg.gather .enable   = 0;
	mem2mem_info_cfg.int_enable = 0; 
	...
	dmac_16bit_mem2mem_cfg(&mem2mem_info_cfg, channel);
