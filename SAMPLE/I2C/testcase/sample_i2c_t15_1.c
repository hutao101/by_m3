...
* Function  Interrupt_I2C2:I2C_RX_UNDER[0]/I2C_RX_FULL[2]/I2C_TX_EMPTY[4]/I2C_RD_REQ[5]/I2C_TX_ABRT[6]/
...
void sample_i2c_t15_1(void)
{
	...
	for(i=0;i<10;i++)
	{
		...
		snps_i2c_config(&i2c1_info, &icp);    //i2c1做主机  传输模式/启主禁从/速度/TAR  //[4]:TX_EMPTY,1
		...
		write_ahb32(i2c2_info.base_address +  I2C_INTR_MASK,status & irq);

		snps_i2c_write_1(&i2c1_info);  //[2]:RX_FULL [8]:ACTIVITY ([9]:STOP_DET) [10]:START_DET  (只能用_1函数写，不然i2c会关闭)
		read_ahb32(i2c2_info.base_address +  I2C_DATA_CMD);
		read_ahb32(i2c2_info.base_address +  I2C_DATA_CMD);//[0]:RX_UNDER 1
		write_ahb32(i2c1_info.base_address +  I2C_DATA_CMD,I2C_CMD | I2C_STOP);  //[5]:RD_REQ [9]:STOP_DET [12]:RESTART_DET
		
		if(i!=9)
		{
			while((read_ahb32(i2c2_info.base_address +  I2C_RAW_INTR_STAT) & I2C_RD_REQ) == 0);//无此条语句会产生中断[6]:TX_ABRT,		
		}
		
		if(read_ahb32(i2c2_info.base_address +  I2C_RAW_INTR_STAT) & I2C_TX_ABRT);         //接上：原因：I2C1读命令还没生效（I2C2读请求还没起来)I2C2就开始写数据了
		{
